...
The following exercises parallel the exercises following Chapter 7 from the textbook (An introduction to Population Genetics - Nielsen and Slatkin). This document can be updated with comments and be compiled to a pdf version using the Knit command. Feel free to do it so! Good luck! :)
...
________________________________________________________still messed up for now
...
# ***** there is no info on selection, so we can probably assume that s=0 (which is what confused us all on Wednesday)
# from Box 7.1:
time_gen <- log(abs((f_A0/f_At - f_A0)/(1-f_A0)))
print(time_gen)
...
_____________________________________________________________________________________ same as for 7.2
...
s_coef <- 0.1
t_gen <- log((f_B0/f_B1 - f_B0)/(1-f_B0))/log(1-s)
print(t_gen)
